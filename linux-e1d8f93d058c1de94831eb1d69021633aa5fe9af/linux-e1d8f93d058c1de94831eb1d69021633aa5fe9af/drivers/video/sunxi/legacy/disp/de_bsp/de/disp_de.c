...
__s32 bsp_disp_set_output_csc(__u32 screen_id, __disp_out_csc_type_t out_type, __u32 drc_csc)
{
	...
	if(out_type == DISP_OUT_CSC_TYPE_HDMI_YUV) {
		...
		enhance_en = 0;
		bright = gdisp.screen[screen_id].lcd_cfg.lcd_bright;
		contrast = gdisp.screen[screen_id].lcd_cfg.lcd_contrast;
		saturation = gdisp.screen[screen_id].lcd_cfg.lcd_saturation;
		hue = gdisp.screen[screen_id].lcd_cfg.lcd_hue;
		out_csc = 0;
	}
