...
Android_OnPadDown(int device_id, int keycode)
{
    ...
    if (SDL_getenv("SDL_JOYSTICK_DEBUG")) {
        SDL_Log("pad down, keycode %i -> button %i", keycode, button);
    }
    if (button >= 0) {
...
Android_AddJoystick(int device_id, const char *name, SDL_bool is_accelerometer, int nbuttons, int naxes, int nhats, int nballs)
{
    ...
    SDL_zero( guid );
    int namelen = SDL_strlen(name);
    *((unsigned char *)&guid) = (unsigned char)namelen;
    SDL_memcpy( ((char *)&guid) + 1, name, SDL_min( sizeof(guid) - 1, namelen ) );
