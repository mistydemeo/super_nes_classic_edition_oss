...
namespace SoLoud
{
	...
	public:
		...
		virtual unsigned int getSamplePosition();
		virtual ~WavStreamInstance();
	private:
		void getWavData(File * aFile, float * aBuffer, unsigned int aSamples, unsigned int aPitch, unsigned int aChannels, unsigned int aSrcChannels, unsigned int aBits);
	};
	...
	public:
		...
		unsigned int mLoopStartOffset;
		unsigned int mLoopEndOffset;
		...
		virtual unsigned getSampleCount() { return mSampleCount; }
