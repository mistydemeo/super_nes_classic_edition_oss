...
mFileHandle(fp)
{
	...
	template<typename T>
	T MemoryFile::readValue()
	{
		T value = T();

		if (mOffset + sizeof(T) <= mDataLength)
		{
			value = *reinterpret_cast<T*>(mDataPtr + mOffset);
			mOffset += sizeof(T);
		}
	...
	unsigned int MemoryFile::read8()
	{
		return static_cast<unsigned int>(readValue<uint8_t>());
	}

	unsigned int MemoryFile::read16()
	{
		return static_cast<unsigned int>(readValue<uint16_t>());
	}

	unsigned int MemoryFile::read32()
	{
		return static_cast<unsigned int>(readValue<uint32_t>());
	}
	...
	unsigned char * MemoryFile::map(size_t length)
	{
		if (mOffset + length <= mDataLength)
			return mDataPtr + mOffset;
