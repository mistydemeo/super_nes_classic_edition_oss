...
namespace SoLoud
{
	...
	time Soloud::getTimePosition(handle aVoiceHandle)
	{
		lockAudioMutex();
		int ch = getVoiceFromHandle(aVoiceHandle);
		if (ch == -1) 
		...
		double v = mVoice[ch]->getSamplePosition() / mVoice[ch]->mBaseSamplerate;
		unlockAudioMutex();
		return v;
	}

	unsigned int Soloud::getSamplePosition(handle aVoiceHandle)
	{
		lockAudioMutex();
		int ch = getVoiceFromHandle(aVoiceHandle);
		if (ch == -1) 
		...
		unsigned int v = mVoice[ch]->getSamplePosition();
		unlockAudioMutex();
		return v;
	}
