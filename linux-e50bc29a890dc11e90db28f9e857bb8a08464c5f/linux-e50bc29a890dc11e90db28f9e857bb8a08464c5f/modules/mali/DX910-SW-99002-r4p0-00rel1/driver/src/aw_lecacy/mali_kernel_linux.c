...
#ifdef CONFIG_HAS_EARLYSUSPEND
static int mali_driver_early_suspend_scheduler(struct device *dev);
static int mali_driver_late_resume_scheduler(struct device *dev);
#endif
...
extern int mali_platform_adjust_gpu_freq(int on);
...
extern unsigned long get_fastboot_mode(); 
#ifdef CONFIG_HAS_EARLYSUSPEND
static int mali_driver_early_suspend_scheduler(struct device *dev)
{
	if(!get_fastboot_mode())
	{
		mali_platform_adjust_gpu_freq(1);
		printk(KERN_ERR "fasboot mode is true\n");
	}
	else
	{

		printk(KERN_ERR "fasboot mode is false\n");
	}
	return 0;
}
...
static int mali_driver_late_resume_scheduler(struct device *dev)
{
	mali_platform_adjust_gpu_freq(0);
	return 0;
}
#endif
static int mali_driver_suspend_scheduler(struct device *dev)
