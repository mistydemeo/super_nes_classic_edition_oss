...
#endif
extern int sunxi_mali_platform_device_register(void);
...
#endif
    err = sunxi_mali_platform_device_register();
    if (0 != err){
...
static int mali_driver_suspend_scheduler(struct device *dev)
{
	MALI_PRINT(("mali_driver_suspend_scheduler\n"));
	mali_pm_os_suspend();
	disable_gpu_clk();
	return 0;
}
...
static int mali_driver_resume_scheduler(struct device *dev)
{
	MALI_PRINT(("mali_driver_resume_scheduler\n"));
	enable_gpu_clk();
	mali_pm_os_resume();
	return 0;
}
