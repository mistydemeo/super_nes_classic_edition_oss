...
SDL_EVDEV_sync_device(SDL_evdevlist_item *item) 
{
    ...
    ioctl(item->fd, EVIOCGKEY(sizeof(keyvalue)), keyvalue);

    int code;
    for (code = 0; code < KEY_MAX; ++code) {
        if (!(keyvalue[code / 8] & (1 << (code % 8))))
            continue;

        if (code >= BTN_MOUSE && code < BTN_MOUSE + SDL_arraysize(EVDEV_MouseButtons)) {
            int mouse_button = code - BTN_MOUSE;
            SDL_SendMouseButton(mouse->focus, mouse->mouseID, SDL_PRESSED, EVDEV_MouseButtons[mouse_button]);
            break;
        }

        SDL_Scancode scan_code = SDL_SCANCODE_UNKNOWN;
        if (code < SDL_arraysize(EVDEV_Keycodes)) {
            scan_code = EVDEV_Keycodes[code];
        }
        if (scan_code != SDL_SCANCODE_UNKNOWN) {
            SDL_SendKeyboardKey(SDL_PRESSED, scan_code);
        }
