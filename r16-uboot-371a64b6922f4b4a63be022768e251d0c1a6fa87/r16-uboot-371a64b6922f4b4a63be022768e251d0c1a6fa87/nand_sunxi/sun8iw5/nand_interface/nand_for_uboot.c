...
extern uint NAND_UbootSimpleRead(uint start, uint blocks, void *buffer);
extern uint NAND_UbootWrite(uint start, uint sectors, void *buffer);
extern uint NAND_UbootSimpleWrite(uint start, uint blocks, void *buffer);
...
uint nand_uboot_phy_read(uint start_block, uint blocks, void *buffer)
{
    int ret;

    if(!NAND_PhyInit())
    {
        ret = NAND_UbootSimpleRead(start_block, blocks, buffer);
    }
    else
    {
        ret = -1;
...
uint nand_uboot_phy_read_noinit(uint start_block, uint blocks, void *buffer)
{
    return NAND_UbootSimpleRead(start_block, blocks, buffer);
}

uint nand_uboot_phy_write(uint start_block, uint blocks, void *buffer)
{
    int ret;

    if(!NAND_PhyInit())
    {
        ret = NAND_UbootSimpleWrite(start_block, blocks, buffer);
    }
    else
    {
        ret = -1;
...
uint nand_uboot_phy_write_noinit(uint start_block, uint blocks, void *buffer)
{
    return NAND_UbootSimpleWrite(start_block, blocks, buffer);
}
