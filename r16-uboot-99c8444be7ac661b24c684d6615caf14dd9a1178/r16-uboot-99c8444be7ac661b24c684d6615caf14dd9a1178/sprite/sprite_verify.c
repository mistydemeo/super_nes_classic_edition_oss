...
************************************************************************************************************
*/
uint sunxi_sprite_part_rawdata_verify_buffer(uint base_start, long long base_bytes, void* buffer)
{
	uint checksum = 0;
    ...
    {
        memcpy(tmp_buf, (u8*)buffer + crt_start*512, VERIFY_ONCE_SECTORS*512);
    ...
    if(rest_sectors)
    {
        memcpy(tmp_buf, (u8*)buffer + crt_start*512, rest_sectors*512);
    	if(unaligned_bytes)
    	{
...
#ifdef CONFIG_SUNXI_SPRITE_SPARSE
