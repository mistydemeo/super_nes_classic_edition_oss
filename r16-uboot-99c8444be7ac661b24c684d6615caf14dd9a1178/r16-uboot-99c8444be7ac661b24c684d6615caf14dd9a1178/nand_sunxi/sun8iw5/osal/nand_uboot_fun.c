...
uint NAND_UbootSimpleRead(uint start, uint blocks, void* buffer)
{
	printf("%s:%d %s, start: %u blocks: %u\n", __FILE__, __LINE__, __func__, start, blocks);
...
uint NAND_UbootSimpleWrite(uint start, uint blocks, void *buffer)
{
	printf("%s:%d %s, start: %u blocks: %u\n", __FILE__, __LINE__, __func__, start, blocks);
    ...
    for (i = start; i < block_count && block < blocks; i++)
    {
        ...
        for (k = 0; k < pages_per_block; k++)
        {
            ...
            if (PHY_SimpleWrite(&para) < 0)
            {
                printf("Warning. Fail in write page %d in block %d.\n", k, i);
                mark_bad_block(para.chip, para.block);
