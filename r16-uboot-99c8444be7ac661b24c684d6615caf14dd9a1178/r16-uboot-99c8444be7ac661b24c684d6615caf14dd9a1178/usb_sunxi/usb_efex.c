...
static u8* raw_boot_block_buffer;
...
static int sunxi_efex_init(void)
{
	...
	raw_boot_block_buffer = (u8 *)malloc(CONFIG_NAND_BOOT_PART_BLOCK_COUNT * CONFIG_NAND_SECTORS_PER_BLOCK * 512);
	if(!raw_boot_block_buffer)
	{
		printf("sunxi usb efex err: unable to malloc memory for raw_boot_block_buffer\n");
...
************************************************************************************************************
*/
static int flashing_raw_boot_partition = 0;

extern uint nand_uboot_phy_write_noinit(uint start_sector, uint sectors, void *buffer);
...
//			break;
		...
		case FEX_CMD_fes_verify_value:
			sunxi_usb_dbg("FEX_CMD_fes_verify_value\n");
			{
				...
				if (flashing_raw_boot_partition == 1 && cmd_verify->start == CONFIG_NAND_BOOT_PARTITION_OFFSET)
				{
					verify_data->media_crc = sunxi_sprite_part_rawdata_verify_buffer(0, cmd_verify->size, raw_boot_block_buffer);
					verify_data->flag 	   = EFEX_CRC32_VALID_FLAG;

					printf(">>>> END RAW BOOT FLASH MODE %s:%d %s\n", __FILE__, __LINE__, __func__);
					flashing_raw_boot_partition = 2;
				}
				else
				{
					verify_data->media_crc = sunxi_sprite_part_rawdata_verify(cmd_verify->start, cmd_verify->size);
...
static void dram_data_recv_finish(uint data_type)
{
	...
	else if(data_type == SUNXI_EFEX_BOOT1_TAG)	//传输BOOT1已经完成
	{
		...
		if (flashing_raw_boot_partition == 2) {
			sunxi_sprite_phywrite(CONFIG_NAND_BOOT_PART_START_BLOCK,
			                      CONFIG_NAND_BOOT_PART_BLOCK_COUNT,
			                      raw_boot_block_buffer);
		}
	}
	else if(data_type == SUNXI_EFEX_BOOT0_TAG)	//传输BOOT0已经完成
	{
	  	...
	  	case SUNXI_USB_EFEX_RECEIVE_DATA:
			...
			if(sunxi_usb_efex_write_enable == 1)		//数据部分接收完毕
			{
				...
				else if(sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_RECEIVE_DATA)//来自于命令阶段，要求接收数据，下一阶段只能是发送状态(status_t)
				{
					...
					else		//表示当前数据需要写入flash
					{
						...
						if (trans_data.flash_start == CONFIG_NAND_BOOT_PARTITION_OFFSET)
						{
							printf(">>>> BEGIN RAW BOOT FLASH MODE %s:%d %s\n", __FILE__, __LINE__, __func__);
							flashing_raw_boot_partition = 1;
						}

						if (flashing_raw_boot_partition == 1)
						{
							if ((trans_data.flash_start - CONFIG_NAND_BOOT_PARTITION_OFFSET + trans_data.flash_sectors) > CONFIG_NAND_BOOT_PART_BLOCK_COUNT * CONFIG_NAND_SECTORS_PER_BLOCK)
							{
								printf("sunxi usb efex err: write flash from 0x%x, sectors 0x%x failed\n", trans_data.flash_start, trans_data.flash_sectors);
							}
							else
							{
								memcpy(raw_boot_block_buffer + (trans_data.flash_start - CONFIG_NAND_BOOT_PARTITION_OFFSET) * 512,
								       (void *)trans_data.act_recv_buffer,
								       trans_data.flash_sectors * 512);
							}
						}
						else if(!sunxi_sprite_write(trans_data.flash_start, trans_data.flash_sectors, (void *)trans_data.act_recv_buffer))
						{
							printf("sunxi usb efex err: write flash from 0x%x, sectors 0x%x failed\n", trans_data.flash_start, trans_data.flash_sectors);
							csw.status = -1;
							trans_data.last_err = -1;
            ...
            case SUNXI_USB_EFEX_RECEIVE_DATA_NEW:
            {
                ...
                else        //表示当前数据需要写入flash
                {
					...
					{
						printf(">>>> BEGIN RAW BOOT FLASH MODE %s:%d %s\n", __FILE__, __LINE__, __func__);
						if (flashing_raw_boot_partition == 0)
						{
							flashing_raw_boot_partition = 1;
						}
					}
