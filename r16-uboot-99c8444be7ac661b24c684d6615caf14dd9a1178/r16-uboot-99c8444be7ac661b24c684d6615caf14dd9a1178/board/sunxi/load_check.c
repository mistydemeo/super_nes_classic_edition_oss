...
#endif
#ifdef CONFIG_USB_DEVICE_EFEX_TEST
...
static int board_probe_power_switch_status(void){
	int ret;
	script_gpio_set_t  gpio;
	__u32  gpio_hd, value = -1;
	ret = script_parser_fetch("platform", "power_switch", (void *)&gpio, sizeof(script_gpio_set_t)/sizeof(int));
	if(ret >= 0)
    {
    	gpio_hd = gpio_request((void *)&gpio, 1);
		ret = gpio_set_one_pin_io_status(gpio_hd, 0, 0);
		printf("gpio_set_one_pin_io_status ret = %d\n", ret);
        value = gpio_read_one_pin_value(gpio_hd, 0);
		printf("gpio_read_one_pin_value value = %d\n", value);
		if(value == 1){
			printf("power switch off\n");
		}else if(value == 0){
			printf("power switch on\n");
		}
	}
	return value;
}

static void board_power_led_ctrl(int sts){
	int ret;
	script_gpio_set_t gpio;
	__u32  gpio_hd;

	ret = script_parser_fetch("platform", "power_led", (void *)&gpio, sizeof(script_gpio_set_t)/sizeof(int));
	if(ret >= 0){
		gpio_hd = gpio_request((void *)&gpio, 1);
		ret = gpio_set_one_pin_io_status(gpio_hd, 1, 0);
		debug("board_power_led_ctrl:gpio_set_one_pin_io_status ret = %d\n", ret);
		ret = gpio_write_one_pin_value(gpio_hd, sts, 0);
		debug("board_power_led_ctrl:gpio_write_one_pin_value ret = %d\n", ret);
	}
...
void board_status_probe(int standby_mode)
{
	...
	else
	{
		...
		//if(bat_exist <= 0)
		power_switch = board_probe_power_switch_status();
		if(power_switch == 1)
		{
			tick_printf("do shutdown \n");
			do_shutdown(NULL, 0, 1, NULL);
			return;
		}
	}
