...
{
	return nand_uboot_phy_read(start_block, nblock, buffer);
}

static int
sunxi_flash_nand_phywrite(uint start_block, uint nblock, void *buffer)
{
	return nand_uboot_phy_write(start_block, nblock, buffer);
}
...
int sunxi_flash_boot_handler(int workmode)
{
...
#ifdef CONFIG_MMC
	int card_no, sdc0_used, sdc_detmode=3;
#endif
	int spi_used=0;
	...
	else
	{
		...
		tick_printf("NAND: ");
		if (skip_init == 0 && workmode == WORK_MODE_BOOT) {
		...
		else if (skip_init == 0 && workmode == WORK_MODE_SPRITE_RECOVERY)
		{
		...
		sunxi_flash_phyread_pt  = sunxi_flash_nand_phyread;
		sunxi_flash_phywrite_pt = sunxi_flash_nand_phywrite;
		sunxi_sprite_phyread_pt  = sunxi_flash_nand_phyread;
		sunxi_sprite_phywrite_pt = sunxi_flash_nand_phywrite;
	}
